...
class Constellation {
    ...
    public function getAll() {
        try {
            error_log("Constellation::getAll - Ejecutando consulta");
            $query = "SELECT * FROM " . $this->table_name . " ORDER BY latin_name";
            $stmt = $this->conn->prepare($query);
            
            if (!$stmt) {
                throw new Exception("Error al preparar consulta: " . implode(", ", $this->conn->errorInfo()));
            }
            ...
            error_log("Constellation::getAll - " . count($data) . " registros obtenidos");
            
            return $data;
            
        } catch (Exception $e) {
            error_log("Error en Constellation::getAll: " . $e->getMessage());
            throw $e;
        }
    }
