...
abstract class BaseController {
    ...
    /**
     * Obtiene y valida el token JWT (desde header Authorization prioritariamente)
     */
    protected function requireAuth() {
        $token = null;
        $tokenSource = '';
        
        // Intentar obtener token desde header Authorization PRIMERO
        ...
        // FALLBACK: Si no hay header, intentar desde cookie (solo para compatibilidad)
        if (!$token) {
        ...
        if (!$token || $token === 'null' || $token === 'undefined' || trim($token) === '') {
            error_log("AUTH DEBUG: Token inválido detectado: '$token'");
            http_response_code(401);
            echo "ERROR: No hay token de autenticación válido. Token recibido: '$token'. Verifica que el token se almacenó correctamente después del login.";
            exit;
        }
