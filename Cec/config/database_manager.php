...
class DatabaseManager {
    ...
    $host = $_ENV['SQLSERVER_HOST'] ?? '88.24.21.189\SPACE';
        $port = $_ENV['SQLSERVER_PORT'] ?? '1433';
        
        // Para instancias nombradas, no agregar puerto si ya está en el host
        if (strpos($host, '\\') !== false) {
            $this->server = $host; // Instancia nombrada, no agregar puerto
        } else {
            $this->server = $host . ',' . $port; // Instancia por defecto, agregar puerto
        }
